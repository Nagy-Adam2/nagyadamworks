...
function plus($conn, $chara_name, $plus)
{
	...
	{
		$comm = "UPDATE characters_page SET chara_exp_point=chara_exp_point+".$plus." WHERE chara_name='".$chara_name."';";
		echo $comm;
	...
	header("Location: Control-buttons-plus.php");
}
?>
...
<head>
	<meta charset="UTF-8" />
	<title>Control buttons plus</title>
</head>
<body>
	<div>
	<?php if(!isset($_POST['chara_name'])!=""):  ?>
	<form action="Control_buttons_plus.php" method="POST">
		<table><h1>Control buttons plus</h1>
		<tr><td>Character name:</td>
			<td><?php
				...
				echo '</select>';
				?></td></tr>
		<tr><td><input type="hidden" name="plus" value="10"></td></tr>
		<tr><td><input type="submit" value="Plus 10"></td></tr>
	</form>
	<?php
	else:
	plus($conn, $_POST['chara_name'],$_POST['plus']);
	endif;
	?>
	</div>
</body>
