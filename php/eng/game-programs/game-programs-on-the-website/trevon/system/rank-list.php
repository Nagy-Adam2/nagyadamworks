...
<head>
	...
	<style type="text/css">
	table.rank {color: #fffcbb; background-color: #6b4542; border-color: #fffcbb; border-style: solid; border-width: 1px;}
	table.rank th {border-left: 1px #fffcbb solid;  border-right: 1px #fffcbb solid; border-bottom: 1px #fffcbb solid;}
	</style>
    <script>
      function frissit(ujra) {
        ...
        chatbox.abort();
        chatbox.open("GET", "rank-list.php?ido="+idopont, true);
        chatbox.onreadystatechange = function() {
          if(chatbox.readyState == 4) {
            document.getElementById('rank').innerHTML = chatbox.responseText;
          }
        }
        chatbox.send(null);
      ...
      function enter_nyomva(e) {
        ...
        if(charCode == 13) {
          ajax_futtat('rank-list.php','mehet=1&szoveg='+document.getElementById('rank').value);
